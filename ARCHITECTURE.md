...
│                                                                     │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────┐  ┌────────┐ │
│  │ Text     │  │ Photo    │  │ Voice    │  │ Stop   │  │ + New  │ │
│  │ input    │  │ upload   │  │ button   │  │ button │  │  trip  │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └───┬────┘  └───┬────┘ │
│       │              │             │             │           │      │
│       │              │        ┌────▼────┐        │           │      │
│       │              │        │ Browser │        │           │      │
│       │              │        │ STT or  │        │           │      │
│       │              │        │ POST    ├──►/transcribe      │      │
│       │              │        │ server  │  (gemini/deepgram) │      │
│       │              │        └────┬────┘        │           │      │
│       │              │     transcript            │           │      │
│       ▼              ▼             ▼              │           │      │
│    { type: "prompt", text, images, fast }    "cancel"    "reset"   │
│       │                                          │           │      │
│       └──────────── WebSocket ───────────────────┴───────────┘      │
│                          ▲                                          │
│  ┌───────────────────────┤  streaming events:                      │
│  │ StreamingMessage      │  delta, reset, tool_start, tool_end,    │
│  │ (progressive render)  │  response_start, response_end, error    │
│  └───────────────────────┘                                          │
└─────────────────────────────────────────────────────────────────────┘
                           │
                     ══════╪══════════════════════════════════════
                           │
┌──────────────────────────▼──────────────────────────────────────────┐
│  SERVER (Express + WebSocket)                           server.ts   │
│                                                                     │
│  Per connection:                                                    │
│    1 pipeline instance (= 1 pi-agent session + intake state)       │
│    busy guard (one turn at a time)                                  │
│    turn buffer (for feedback)                                       │
│                                                                     │
│  "cancel" → pipeline.abort()                                       │
│  "reset"  → pipeline.dispose() + create new pipeline               │
│  "prompt" ──▼                                                       │
└─────────────┼───────────────────────────────────────────────────────┘
              │
┌─────────────▼───────────────────────────────────────────────────────┐
│  PIPELINE (the conversation engine)                   pipeline.ts   │
│                                                                     │
│  State: userTurns[], photoLocationHint, askedIntake                │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────┐       │
│  │  STEP 1: INTAKE GATE                        intake.ts   │       │
│  │                                                         │       │
│  │  extractIntake(turns) ─── plain fetch to OpenRouter     │       │
│  │    • text-only → default model (Haiku)                  │       │
│  │    • has image → VISION_MODEL (Gemini Flash)            │       │
│  │    • returns: { intent, start_location, days,           │       │
│  │                 start_date, date_conflict, in_scope }   │       │
│  │    • validated through TypeBox schema                   │       │
│  │                                                         │       │
│  │  gateDecision():                                        │       │
│  │    missing start or days, or date conflict?             │       │
│  │      YES (first time) → templated question, RETURN      │       │
│  │      YES (second time) → apply defaults, CONTINUE       │       │
│  │    non-NL photo? → redirect message, RETURN             │       │
│  │    refinement? → SKIP gate entirely                     │       │
│  └────────────────────────┬────────────────────────────────┘       │
│                           │ params settled                          │
│  ┌────────────────────────▼────────────────────────────────┐       │
│  │  STEP 2: PLANNING (pi-agent ReAct loop)                 │       │
│  │                                                         │       │
│  │  session.prompt(                                        │       │
│  │    [buffered texts] + [confirmed params line] + text,   │       │
│  │    { images }                                           │       │
│  │  )                                                      │       │
│  │       │                                                 │       │
│  │       ▼                                                 │       │
...
│  │                                                         │       │
│  │  • Empty turn (<20 chars)? → SYNTHESIS_REPROMPT         │       │
│  │  • Zero tools + asking reply? → CLARIFICATION_REPROMPT  │       │
│  │  • Leaked preamble? → stripReasoningPreamble()          │       │
│  └─────────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────────┘
              │
              │  tool execute() calls
...
│  7 TOOLS                                             tools/*.ts     │
│                                                                     │
│  geocode ──────────► Nominatim (NL-scoped)                         │
│  plan_route ───────► OpenRouteService (cycling) / OSRM (fallback)  │
│  get_weather ──────► Open-Meteo                                    │
│  find_pois ────────► Overpass (local Docker or public)             │
│  find_accommodation► Overpass                                       │
│  find_knooppunten ─► Overpass                                       │
│  web_search ───────► DuckDuckGo (bounded, discouraged)             │
│                                                                     │
│  Overpass calls go through a shared client:                         │
│    in-memory cache → serialized queue → bounded backoff             │
└─────────────────────────────────────────────────────────────────────┘
```
