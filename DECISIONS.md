...
- **Interface: minimal web chat** (Express + WebSocket + vanilla JS): streaming, tool-activity chips, Fast mode (default), image upload, **Stop** (cancel an in-flight plan via the SDK's `session.abort()`), **New trip** (fresh session — the latency reset for long conversations), and voice input. The agent's input is always **text + image**; voice is transcribed to text first — by the browser (default) or, via `STT_BACKEND`, a server STT (Deepgram, or Gemini on the same OpenRouter key) for better accuracy — so the grounding rules apply to every modality.
- **Photo → start city via a vision model.** A photo's city is identified by the intake step using `VISION_MODEL` (default Gemini 2.5 Flash; a small text model misnames Dutch canal towns and even returns foreign cities). The identification is *disclosed for confirmation*, never silently committed, and a clearly non-Dutch photo is redirected rather than coerced into an NL city.
